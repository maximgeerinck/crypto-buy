...
class SharePortfolio extends Component {
    ...
    render() {
        ...
        const bound = PortfolioHelper.bindPortfolioToCoin(
            this.props.portfolio.coins.get("items"),
            this.props.coins.coins.toObject(),
        );

        let shareContainers = shares.map(share => {
            const permissions = Object.keys(share).filter(
                key => typeof share[key] === "boolean" && share[key],
            );
            return (
                <ShareItem
                    key={share.id}
